...
struct Auth_MSG *deserialize_auth_msg(uint8_t *buffer, const size_t buffer_size)
{
    ...
    size_t username_len = strlen((char*)&buffer[offset])+1; // null char
    auth_msg->username = malloc(username_len * sizeof(char));
    if (auth_msg->username == NULL) {
        free(auth_msg);
        perror("Memory allocation failed");
        exit(EXIT_FAILURE);
    }
    strcpy(auth_msg->username, (char*)&buffer[offset]);

    offset += username_len;

    size_t display_name_len = strlen((char*)&buffer[offset])+1;
    auth_msg->display_name = malloc(display_name_len * sizeof(char));
    ...
    strcpy(auth_msg->display_name, (char*)&buffer[offset]);

    offset += display_name_len;

    size_t secret_len = strlen((char*)&buffer[offset])+1;
    auth_msg->secret = malloc(secret_len * sizeof(char));
    ...
    strcpy(auth_msg->secret, (char*)&buffer[offset]);
...
struct Join_MSG *deserialize_join_msg(uint8_t *buffer, const size_t buffer_size)
{
    ...
    size_t channel_id_len = strlen((char*)&buffer[offset])+1;
    join_msg->channel_id = malloc(channel_id_len * sizeof(char));
    ...
    strcpy(join_msg->channel_id, (char*)&buffer[offset]);

    offset += channel_id_len;

    size_t display_name_len = strlen((char*)&buffer[offset])+1;
    join_msg->display_name = malloc(display_name_len * sizeof(char));
    ...
    strcpy(join_msg->display_name, (char*)&buffer[offset]);
